<script lang="ts">
  ...
  onMount(() => {
    setVoice({
      state: 'idle',
      hint: 'Diga algo — a Íris responde no tom que você escolher',
      level: 0.5
    })
  })
</script>
...
<div class="page">
  ...
  <div class="body">
      ...
      <div class="group">
        ...
        <div class="aura">
          ...
          <div class="track">
            ...
            <div class="knob" style="left:74%"></div>
          </div>
        </div>
      </div>
    ...
    <div class="stage">
      ...
      <div class="caption">
        <p>
          "Oi, Renato. Sou a {name || 'Íris'} — vou te ajudar a manter tudo em ordem, no seu ritmo."
        </p>
      </div>
      <button class="test"
        ><Icon name="mic" size={17} stroke="var(--purple-glow)" /> Testar voz</button
      >
    </div>
  </div>
</div>
...
<style>
  .page {
    ...
    height: 100%;
    background:
      radial-gradient(circle at 72% 36%, rgba(139, 92, 246, 0.12), transparent 55%), var(--bg-base);
  }
  .head {
    display: flex;
  ...
  .stage {
    ...
    overflow: hidden;
    background:
      radial-gradient(circle at 50% 40%, rgba(167, 139, 250, 0.2), transparent 58%),
      var(--surface-sunken);
    display: flex;
    flex-direction: column;
    align-items: center;

<script lang="ts">
  ...
  const activeNode = $derived(g?.nodes.find((n) => n.id === active))
  const neighbors = $derived.by<GraphNode[]>(() => {
    if (!g) return []
...
<div class="page">
  <header class="head">
    ...
    <div class="search">
      <Icon name="search" size={15} stroke="var(--text-2)" /> Buscar entidade
    </div>
  </header>
      ...
      {#if activeNode}
        <div class="detail">
          <div class="d-head">
            <span
              class="d-bead"
              style="background:{col(activeNode.color)};box-shadow:0 0 12px {col(activeNode.color)}"
            ></span><span class="d-name">{activeNode.label}</span>
          </div>
          <div class="overline">
            {activeNode.kind === 'context'
              ? 'Empresa · contexto'
          ...
          <div class="conns">
            {#each neighbors.slice(0, 4) as n (n.id)}
              <div class="conn">
                <span class="cb" style="background:{col(n.color)}"></span><span class="cn"
                  >{n.label}</span
                ><span class="ck mono">{kindLabel[n.kind] ?? n.kind}</span>
              </div>
            {/each}
          </div>

<script lang="ts">
  ...
  onMount(async () => {
    setVoice({
      state: 'transcribing',
      transcript: 'na Cogna tem daily de 9:30 às 10:00, recorrente',
      level: 0.6
    })
    try {
  ...
  <div class="body">
    <div class="timeline-card">
      <div class="tl-head">
        ...
        <span class="chip danger"
          ><span class="d"></span>{data?.conflicts ?? 1} conflito às 14:00</span
        >
      ...
      <div class="grid">
        <div class="rail" style="height:{H}px">
          ...
          {#each data?.events ?? [] as ev (ev.id ?? ev.title)}
    ...
    <div class="side">
      <div class="voice-card">
        <div class="vc-head">
          <span class="overline" style="color:var(--purple-glow)">Criar por voz</span>
        </div>
        <p class="vc-text">"na Cogna tem <b>daily</b> de <b>9:30 às 10:00</b>"</p>
        <div class="struct">
          <div class="struct-head">
            <span class="struct-title">Daily</span><ContextChip
              label="Cogna"
              color="violet"
              size="sm"
            />
          </div>
          <div class="struct-time mono">
            <Icon name="clock" size={14} stroke="var(--text-3)" /> 09:30–10:00 · recorrente, seg–sex
          </div>
        </div>
      ...
      <div class="detail-card">
        ...
        <div class="d-row">
          <Icon name="users" size={15} stroke="var(--text-3)" /><span>Marina, Téo, Renato · +2</span
          >
        </div>
        <div class="d-row">
          <Icon name="file" size={15} stroke="var(--text-3)" /><span
            >Íris vai gerar o resumo e os itens de ação ao fim.</span
          >
        </div>
      </div>
    </div>

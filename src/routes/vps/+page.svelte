<script lang="ts">
  ...
  const gauges = $derived([
    {
      label: 'CPU · 8 vCPU',
      detail: v?.cpuDetail ?? 'load 2.1 / 1.8 / 1.5',
      value: v?.cpu ?? 0.42,
      spark: '0,18 15,14 30,19 45,10 60,15 75,8 90,13 105,9 120,12'
    },
    {
      label: 'Memória',
      detail: v?.ramDetail ?? '11.5 / 16 GB',
      value: v?.ram ?? 0.72,
      spark: '0,14 15,13 30,12 45,11 60,12 75,9 90,8 105,7 120,6'
    },
    {
      label: 'Disco · SSD',
      detail: v?.diskDetail ?? '232 / 400 GB',
      value: v?.disk ?? 0.58,
      spark: '0,16 15,16 30,15 45,15 60,14 75,14 90,13 105,13 120,12'
    }
  ])
</script>
...
<div class="page">
  <header class="head">
    ...
    <div class="badges">
      ...
      <span class="chip ok"
        ><span class="d glow"></span>{v?.online === false ? 'offline' : 'online'}</span
      >
    </div>
  </header>
  ...
  <div class="body">
          ...
          <div class="g-inner">
            ...
            <div class="g-meta">
              ...
              <svg
                ...
                fill="none"
                stroke={ringColor[i]}
                stroke-width="1.6"><polyline points={g.spark} /></svg
              >
            </div>
          </div>
        </Card>
    ...
    <div class="cols">
      <div class="containers">
        <div class="ct-head">
          <span class="ct-title">Containers</span><span class="ct-sub mono"
            >{v?.containers.length ?? 5} ativos</span
          >
        </div>
        {#each v?.containers ?? [] as c, i (c.name)}
          <div
            class="crow"
            class:warn={c.status === 'restarting'}
            class:divider={i < (v?.containers.length ?? 0) - 1}
          >
            <span
              class="cdot"
              style="background:{c.status === 'restarting'
                ? 'var(--warning)'
                : 'var(--success)'};box-shadow:0 0 8px {c.status === 'restarting'
                ? 'var(--warning)'
                : 'var(--success)'}"
            ></span>
            <span class="cname">{c.name}</span>
            {#if c.status === 'restarting'}
              <span class="cstatus mono"
                ><span class="spin"
                  ><Icon name="refresh" size={11} stroke="var(--warning)" strokeWidth={2.4} /></span
                >reiniciando</span
              >
            {:else}
            ...
            <button class="cbtn" onclick={() => askRestart(c.name)} aria-label="reiniciar"
              ><Icon
                name="refresh"
                size={15}
                stroke={c.status === 'restarting' ? 'var(--warning)' : 'var(--text-3)'}
              /></button
            >
          </div>
        {/each}
        ...
        {#if confirmTarget}
          <div class="confirm">
            ...
            <div class="cf-body">
              ...
              <div class="cf-sub">
                O container ficará indisponível por ~3s. Diga "confirmar" ou clique.
              </div>
            </div>
            <Button variant="subtle" size="sm" onclick={() => (confirmTarget = null)}
              >Cancelar</Button
            >
      ...
      <div class="logs">
        <div class="logs-head">
          <span class="overline">Logs ao vivo</span><span class="stream mono"
            ><span class="sdot"></span>stream</span
          >
        </div>
        <div class="log-lines">
          {#each v?.logs ?? [] as l, i (i)}
            <div class="log">
              <span class="lt">{l.time}</span><span class="ll" style="color:{levelColor[l.level]}"
                >{l.level.padEnd(4)}</span
              ><span class="lb">{l.body}</span>
            </div>
          {/each}
        </div>
      </div>
